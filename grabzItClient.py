...
class GrabzItClient:
    ...
    def Save(self, callBackURL=''):
        if (self.request == None):
            raise GrabzItException.GrabzItException("No screenshot parameters have been set.",
                                                    GrabzItException.GrabzItException.PARAMETER_MISSING_PARAMETERS)

        sig = self.CreateSignature(
            self.request.options._getSignatureString(self.applicationSecret, callBackURL, self.request._targetUrl()))

        if (self.request.isPost == False):
            return self.GetResultObject(self.HTTPGet(self.request.url + '?' + urlencode(
                self.request.options._getParameters(self.applicationKey, sig, callBackURL, 'url', self.request.data))),
                                        "ID")
        else:
            return self.GetResultObject(self.HTTPPost(self.request.url,
                                                      self.request.options._getParameters(self.applicationKey, sig,
                                                                                          callBackURL, 'html',
        ...
        while (1):
            ...
            if not (status.Cached) and not (status.Processing):
                raise GrabzItException.GrabzItException(
                    "The screenshot did not complete with the error: " + status.Message,
                    GrabzItException.GrabzItException.RENDERING_ERROR)
                break
            elif status.Cached:
                result = self.GetResult(id)
                if result == None:
                    raise GrabzItException.GrabzItException("The screenshot could not be found on GrabzIt.",
                                                            GrabzItException.GrabzItException.RENDERING_MISSING_SCREENSHOT)
                    break
    ...
    def GetCookies(self, domain):
        ...
        for cookieNode in cookieNodes:
            ...
            results.append(GrabzItCookie.GrabzItCookie(self.GetFirstValue(name), self.GetFirstValue(value),
                                                       self.GetFirstValue(domain), self.GetFirstValue(path),
                                                       (self.GetFirstValue(httpOnly) == self.TrueString),
                                                       self.GetFirstValue(expires), self.GetFirstValue(type)))
    ...
    def SetCookie(self, name, domain, value="", path="/", httponly=False, expires=""):
        sig = self.CreateSignature(
            str(self.applicationSecret) + "|" + str(name) + "|" + str(domain) + "|" + str(value) + "|" + str(
                path) + "|" + str(int(httponly)) + "|" + str(expires) + "|0")

        qs = {"key": self.applicationKey, "domain": domain, "name": name, "value": value, "path": path,
              "httponly": int(httponly), "expires": expires}
    ...
    def AddWaterMark(self, identifier, path, xpos, ypos):
        ...
        try:
            ...
            raise GrabzItException.GrabzItException("File: " + path + " does not exist",
                                                    GrabzItException.GrabzItException.FILE_NON_EXISTANT_PATH)

        sig = self.CreateSignature(
            str(self.applicationSecret) + "|" + str(identifier) + "|" + str(xpos) + "|" + str(ypos))
    ...
    def getWaterMarks(self, identifier=""):
        ...
        for waterMarkNode in waterMarkNodes:
            ...
            results.append(
                GrabzItWaterMark.GrabzItWaterMark(self.GetFirstValue(identifier), self.GetFirstValue(xPosition),
                                                  self.GetFirstValue(yPosition), self.GetFirstValue(format)))
    ...
    def CheckResponseHeader(self, httpCode):
        if httpCode == 403:
            ...
            raise GrabzItException.GrabzItException('A network error occured when connecting to the GrabzIt servers.',
